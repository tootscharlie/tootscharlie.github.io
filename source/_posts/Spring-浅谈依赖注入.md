...
tags: Spring
---
Spring框架的核心技术是IoC，将对象的创建时机和创建过程交由IoC容器来管理，并且在容器实例化对象之后自动注入该对象所需的外部依赖，由于整个过程从根本上来说是反向的，所以称该过程为`控制反转`，也叫`依赖注入（DI）`。我在{% post_link Spring:白话一下IoC %}和{% post_link Spring-简单摆弄一下SpringIoC容器 %}两篇文章中简单描述了IoC是什么和Spring的IoC相关使用方式，本篇简单谈一下IoC的另一层意思——依赖注入（DI）
